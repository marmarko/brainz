...
- **Phase 0 cheap-check tasks (founder-owned, parallel to U1):** **two of the six are now done.** ✅ Assumption 4 settled 2026-08-12 (raw TCP works from a deployed Container — KTD2 stands). ✅ R9's storage boundary settled 2026-08-12 (prefix-scoped credentials enforce it; bucket-quota question retired). ✅ Assumption 3 accepted on founder attestation (Claude Desktop does this; U12 re-verifies against the real connector). **The remaining three were deliberately dropped from Phase 0 by the user on 2026-08-12** and each now lands where it will actually be needed: the Pipedream compliance question moves to the top of **U9** (see Assumption 1 — it is the one that can invalidate the alpha bar, so it is asked first there, not last); Workers AI rate limits move to **U10**, where a low per-account cap would mean per-model queueing in the scheduler; and the claude.ai web/mobile connector-surface question moves to **U15**, where it decides which funnel the guided connect flow is designed for. Drafts for the two vendor emails are kept in `docs/vendor/` rather than discarded. Each costs effectively nothing and each can invalidate a load-bearing decision; running them after Phases 0–3 would strand real work. Four need no vendor at all — a stub tool, a throwaway container, an R2 API call and a test account answer them today — and the scheduled-task failure mode is a *different product* ("ask for a briefing" rather than "wake up to one"), which is a Phase 0 decision, not a Phase 3 discovery.
...
- Assumption 1. Pipedream Connect's OAuth apps are usable for production consumer Gmail read scopes under their compliance umbrella, and Pipedream supports programmatic external-user deletion with token revocation (needed for R12's erasure leg). ~~**Verified in writing in Phase 0**~~ — **DELIBERATELY DEFERRED TO U9, 2026-08-12 (user-directed).** The vendor question was drafted (`docs/vendor/2026-08-12-pipedream-compliance.md`) and not sent. So this assumption is now **carried into Phase 2 unverified**, and U9's week-1 task changes from re-verification to first verification. Recorded rather than left implicit because the plan's Phase 0 framing otherwise reads as though this were cleared: it is the one assumption whose failure invalidates the alpha bar instead of complicating it, and the fallback below costs an MBOX parser plus a scope reduction — cheaper to absorb than the alternative, but not free, and worse discovered mid-Phase-2 than before Phase 1 starts. **Ask at the top of U9, not at the end.** Nothing before U9 depends on the answer, which is what makes the deferral affordable; KTD6 itself is settled and unaffected. If the answer is no, the surfaced options are CASA-free scopes (Calendar/Contacts/Drive-picker) **plus an MBOX mailbox-export path through U8's folder import** — the added cost is an MBOX parser. Microsoft Graph is not a fallback for this founder, whose mail is in Gmail. **The alpha bar requires a mail source, connector-fed or export-fed**: U11's contradiction report and the commitments layer are verified against real mail, so shipping alpha without one is a different alpha, not a degraded one.
...
- Assumption 3. Claude Desktop and Claude Code scheduled tasks can invoke a custom-connector MCP tool unattended. This is the entire v1 briefing delivery channel (KTD12/R21). **Accepted 2026-08-12 on founder attestation — Claude Desktop does this.** Note the evidence class honestly: this is operator knowledge of the client, not a measurement, unlike Assumption 4 and R9 which carry probe receipts. It is a reasonable basis to proceed on — the founder uses the client daily — and it is cheap to be wrong about here, because **U12's re-verification against the real `briefing` connector is retained and is where this actually gets proven**. The stub probe built for this (`scripts/probes/scheduled-task/`) is kept unrun: it can only settle the cloud-scheduled case anyway (a Desktop-local schedule needs the machine awake, so every "you weren't there" discriminator is unavailable by construction), and U12 needs a real connector regardless. If it fails, the surfaced Phase 0 decision is the delivery channel itself: a documented manual morning-pull recipe, with push delivery promoted from "revisit later" to a Phase 4 commitment.
...
- **Inherited dependency — the review queue has no close path, and this unit is where one becomes possible.** U11 ships the queue complete on its own terms: mid-confidence proposals land in `review_queue`, and the *closing channel is constrained in the schema* — `review_queue_closed_by_is_out_of_band` admits `user_out_of_band` and `internal` and refuses `agent_mcp`, because R12a says a restatement arriving over `/mcp` clears nothing. That is deliberate and must not be relaxed: the assistant holding `remember` is the assistant reading the user's mail. What is missing is a *surface* the connected agent cannot drive, and the enum this unit declares (`set_context_policy | set_spend_cap | pause_source | resume_source`) contains no review action at all. So the re-plan owes an explicit decision: either add a nonce-gated panel action here — in which case the fallback rule above applies to it with full force, since approving a model-proposed mutation is exactly the class that must never execute unconfirmed — or route the close web-app-only through U15 and say so. Until one of those lands, `pending_review` is a number a user can read and cannot act on; `docs/recipes/weekly-review.md` tells them that in those words, and that paragraph is what goes stale when this is built.
