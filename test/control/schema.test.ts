...
const CONTENT_BEARING_SQL: readonly ContentBearingEntry[] = [
  ...
  },
  {
    ...
    because:
      "rung 19 holds no user content and adds no table: two columns on the tenant's own `consolidation_checkpoint` plus one widened CHECK on `consolidation_run`, both of which rung 3 already classified as operational. `completed` is a boolean and `phase_cursor` is a **row id inside this tenant's own database** — a `page_id` or a `chunk_id`, written and read only by the phase that emitted it — so it is a position rather than a payload, and the CHECK that it be present on an incomplete row is what keeps it from becoming an optional free-text field somebody later parks a filename in. It is named here rather than exempted for the reason rung 4 gives: a rung that is content-free today is exactly the one a later column gets added to, and the temptation on this table specifically is real — the obvious 'improvement' is to record *what* a phase was working on rather than where it was, and a page title in an operational table is a leak with a plausible motive. It is emphatically the tenant's database because the checkpoint has to commit against the same database as the work it describes, which is rung 3's first argument and unchanged. `DROP CONSTRAINT` appears and is admitted by the expand-only scanner only as half of a same-name re-add, because a CHECK cannot be widened any other way, and the replacement adds exactly one label — `out_of_time`, for a cycle that stopped on the attempt's wall clock with work left rather than because a cap fired or a provider was down",
  },
  {
