...
const CONTENT_BEARING_SQL: readonly ContentBearingEntry[] = [
  ...
  },
  {
    ...
    because:
      "rung 21 adds one column to `page`, which is the most content-bearing table in the schema, and it holds none: `consolidation_refusals` is an integer count of how many times a model phase asked the provider to read that page and got nothing usable back. It is emphatically the tenant's database because `page` is where the user's own documents live. The temptation rung 19 warned about and rung 20 met is at its sharpest on this table, because here the row IS the user's document — the obvious 'improvement' on a count is to record what the provider said about it, and a provider's sentence about a page is an excerpt of that page beside the excerpt-free codes the rest of the cycle keeps. A bare integer cannot carry one. The rung's first draft added a second column, `quarantine_reason`, alongside a write that set `page.quarantined_at` after two durable refusals so an unreadable page would leave the summariser's candidate set; both were removed before the rung shipped, and the removal is the point of the entry. `quarantined_at` is U9's junk gate and EVERY read honours it — search, the briefing, and the user's own self-export — so retiring a page there would have taken a document its owner still has out of their own brain, silently, on evidence the phase itself admits it cannot have: an unparseable HTTP 200 is a page the model can never read and one badly sampled answer at the same time. So nothing is retired, the phase completes through the pages it cannot read instead of stopping on them (which is what froze a brain at 5,608 pages and 167 facts, by leaving a run open in front of another phase's checkpoint), and the counter is telemetry an operator reads rather than a threshold anything compares against — the moment something compares it, the refuted draft is back. `integer` rather than smallint for the same reason: a counter that no longer stops at a threshold grows once per cycle for as long as a page stays unreadable, and an overflow inside the phase would be a fresh way for one page to break a run. The rung also drops and re-adds `consolidation_run_stopped_phase_code_is_known` under the same name, which is the only way a CHECK is widened and the one shape the expand-only scanner admits, to add `input_rejected` — the provider refusing the request rather than the connection. That code outlives the draft that motivated it: it is what a per-item phase now uses to tell a failure that is about one request from one that is about the provider, the credential or the configuration and would meet every remaining page identically",
  },
  {
    path: 'schema/migrations/v27-relationships.sql',
    because:
      "rung 27 holds no user content: eight rows in `edge_type`, which rung 2 already classified as a registry of relationship KINDS and their declared inverses, and one nullable bigint on `tenant_setting`. It adds the vocabulary a brain needs to say something every edge type shipped before it could not — that a person is somebody's wife, child, parent, sibling, colleague or friend, and that a company belongs to whoever owns it. It is emphatically the tenant's database because both the relationships and the identification are statements about the owner's own life. The temptation rung 19 named is answered here by shape twice over: an `edge_type` row is a slug, an inverse and a one-line description of the KIND, constrained by `edge_type_is_a_slug` to `^[a-z][a-z0-9_]{1,63}$`, so it cannot hold a name or a sentence about anybody; and `self_entity_id` is a bigint, which has no room for one. `belongs_to` is deliberately NOT folded into `part_of` even though both read as containment in casual English: a subsidiary is a COMPONENT of its parent, which is what the extractor derives from 'X is part of Y' and what `part_of` has always meant, while a company BELONGS TO the person or holding company that owns it — and the object of `belongs_to` may be a person, which no other edge type in this schema permits. Collapsing them would destroy exactly the distinction a day of corrections restored. The rung's most important property is what it does NOT add: no extractor rule, no model phase, no inference of any kind writes these edges. Family cannot be derived from correspondence without the confident guessing that produced a person called `Here` and twelve months filed as organizations, so the only writer is the owner stating it. `self_entity_id` is nullable with no default and no backfill for rung 7's reason applied a fourth time — NULL means the owner has never said which entity is them, which is a different fact from any guess, and writing one in would look like a decision they made on the one column whose whole purpose is recording a decision they made. It carries no foreign key for `correspondent.entity_id`'s reason: the purge hard-DELETEs entities and must never raise, `ON DELETE SET NULL` would turn a mistaken `forget` into 'never chosen' rather than a correctable error, and `widenEntityOrigins` re-points holders by explicit UPDATE — so a pointer at a tombstoned row reads as unset, which is the honest reading of a statement about a row that is no longer there",
  },
  {
    path: 'schema/migrations/v26-correspondent.sql',
    because:
      "rung 26 is the FIRST rung in this ladder to add a table holding third-party personal data the user did not write — email addresses and display names belonging to the people the owner corresponds with — and it is emphatically the tenant's database, one row per address per origin. It carries no page text, no subject line and no body: `display_name` is the spelling a provider attached to an address, `address_key` is that address normalized, and `name_key` exists only because `lower()` is not `normalize()` and a data-subject-rights path must not miss `O’Brien` against `o'brien`. The temptation rung 19 named is at its sharpest here of anywhere in the schema, because this table is ABOUT other people: the obvious 'improvement' on a display name is the sentence the mail said about them, and that is correspondence content in a dictionary. Every content-shaped column is therefore a name or an address and nothing else, and the two counters a naive design would add — `stated_pages`, `last_subject` — are refused by making evidence a JOIN: `correspondent_sighting` holds one row per (correspondent, live page, role), so the count is idempotent under a re-poll and SHRINKS when a page is forgotten, where a stored integer would keep evidence from mail the brain no longer holds. What makes this rung defensible rather than merely careful is the measurement it was designed against: of 2,131 distinct addresses in the owner's address book, THIRTEEN appear anywhere in a 10,036-page corpus, so the dictionary answers 'what is this person called' and never 'does this person exist' — and `correspondent_sighting_role_is_known` omits 'book' precisely so the address book, which carries no page, can never insert a sighting, can never satisfy the evidence test, and can never create a person. That is structural rather than a policy comment somewhere else. `entity_id` deliberately carries NO foreign key, and the reason is that all three of the alternatives fail: a plain FK can make the purge raise, `ON DELETE SET NULL` makes a `forget` un-stick because `forgetRecord` writes no suppression row and a nulled binding reads as never-bound, and a hard reference cannot survive `widenEntityOrigins`' successor cascade — so a binding pointing at a tombstoned or absent entity is READ as a user retraction, on the precedent `review_queue.target_ref` already sets. Neither table carries `deleted_at`, also deliberately: both are hard-deleted by the erasure and severance arms, because a plaintext address sitting soft-deleted for the life of the brain in a table where presence IS the record is the failure wearing the fix's clothes — and the cost of that choice, stated rather than hidden, is that neither table is enrolled in any automatic lifecycle census and the two hard-delete arms are held in place by their own tests alone",
  },
  {
    path: 'schema/migrations/v25-contacts-source.sql',
    because:
      "rung 25 holds no user content, adds no table and adds no column: three CHECKs are dropped and re-added under their own names, which is the only way a CHECK is widened and the one shape the expand-only scanner admits. Every replacement is a strict superset, so no existing row can be invalidated and no tenant's ladder can stall on a scan — the failure mode rung 24's entry describes and refuses to risk. It is the tenant's database because all three constraints guard tenant tables. `source_pause_source_is_a_connector` gains `contacts` because the pause control is offered for every source the connectors page renders, and `pauseSource` binds the source as text rather than casting it, so a paused fourth connector would raise the CHECK the moment somebody pressed the button — loud and repairable, which is why it is the least interesting of the three. The two that deserve the scrutiny are `page_source_type_is_known` and its `page_version` twin, because they are widened for a value that **nothing writes**: the contacts adapter deliberately produces no pages at all. That decision is the rung's whole justification and it is a measurement, not a preference — the address book this was built against holds 2,525 contacts and 2,131 distinct addresses, of which THIRTEEN appear anywhere in the corpus, so contacts are a dictionary consulted about people the brain has independently seen and never a document set. 2,525 pages would bury a 10,036-page corpus under a decade of stale address-book cruft, and 2,525 entities would take a 56-row roster to roughly 2,500 where 99.5% of the rows are attested by nothing — which is precisely the failure the admission fence was built to stop, at fifty times the scale and self-inflicted. The alphabet is widened anyway because a vocabulary the code can name and the database cannot hold is a runtime failure waiting for the first caller who does the obvious thing, and `SOURCE_TYPE_FOR` is a total map over `ConnectorSource` whose fourth entry has to be something. Making that entry `note` would have avoided this file by writing a lie into every ingest-log row the lane produces; cheaper is not the same as true. The temptation rung 19 named is answered here by shape: `source_type` is a closed alphabet of nine literals spelled out in the CHECK itself, and no later edit can put a subject line or a person's name in one without changing the alphabet in front of a reviewer",
  },
  {
    path: 'schema/migrations/v24-review-actor.sql',
    because:
      "rung 24 holds no user content and adds one nullable text column to `contradiction_report`, which rung 2 already classified as operational: `resolved_by`, constrained on arrival to `user_out_of_band` or `internal` and nothing else. The alphabet IS the classification here — the column is a two-member closed vocabulary declared in its own CHECK in this file, so it is the same shape the control plane's domains take and it cannot hold a sentence, a subject line or a name however carelessly a later caller writes to it. The temptation rung 19 named is sharp on this table for the same reason it is sharp on `page`: the row is ABOUT two of the user's own statements, and the obvious 'improvement' on an actor is a note saying why the owner decided as they did — which would be the user's words in a column beside the codes the rest of the table keeps. A closed set of two cannot carry one. It is emphatically the tenant's database because the report has always lived beside the facts it names. The rung exists because R12a is a rule about WHO may close a decision, `review_queue.closed_by` has enforced it in a CHECK since rung 3, and one table over the same question was unanswerable: a resolution written by the owner's own session and one written by a future automated pass were byte-identical rows. That was harmless while nothing could write a non-open row and stops being harmless the moment a second writer exists — `/dashboard?view=review` is that second writer, and what it writes is a VERDICT that touches neither fact, which is how R12's report-only requirement stays true. This column is what lets the next reader verify that rather than take it on trust. The paired constraint requiring `resolved_by` on every non-open row is deliberately NOT here: nothing in `src/` has ever been able to write a non-open row, so any that exist were written by hand, and `ADD CONSTRAINT` validates against every existing row — the constraint would fail its scan on the first tenant holding one, and a rung that fails on one tenant stops the ladder for the whole fleet. Trading a fleet-wide migration stall for a check on a state no code can produce is the wrong side of that bargain; it is an obvious later rung once the existing rows are stamped",
  },
  {
    path: 'schema/migrations/v23-run-closure.sql',
    because:
      "rung 23 holds no user content and adds one nullable timestamp to `consolidation_run`, which rung 3 already classified as operational: `resumed_at`, the instant a later cycle adopted a run whose own cycle never returned. An instant and nothing else — it cannot say which page, which phase or which provider, and the temptation rung 19 named and rungs 20 and 21 met is answered here by shape rather than by discipline, because there is no room in a timestamptz for a sentence. It is emphatically the tenant's database because the run record has always lived beside the work it describes. The rung exists because `finished_at IS NULL` was carrying two meanings at once — a cycle that DECIDED to stop and left work behind, and a cycle that was killed and never wrote anything — and the first one froze a brain: one page drawing a provider 500 stopped the synopsis phase, which stopped the cycle, which left the run open, which left `extract`'s checkpoint standing in front of every later cycle. 5,608 pages behind 167 facts, with extraction called once. A cycle that returns now closes its run whatever stopped it, so an open run means the second meaning alone, and this column is what bounds the resume it licenses to ONE: a worker dying at the same phase every time would otherwise skip the phase it banked forever, which is the same absorbing state through a different door. The rung deliberately comes after rung 22 rather than instead of it: closing the run was built once on its own and reverted, because with four phases holding no record of their own progress it re-paid `extract` every cycle and left `enrich`, `synopsis`, `contradiction` and `salience_refine` unreached — measured, five cycles, zero summaries. Nullable with no backfill, deliberately — NULL means 'never adopted', so a run left open in production is adoptable exactly once and the brain heals itself on its second cycle with nobody touching a row. No CHECK pairs it with `started_at`: the two are written by different workers from their own clocks, and a constraint that turned clock skew into a refused cycle would be this rung's own version of the failure it ends",
  },
  {
    path: 'schema/migrations/v22-considered.sql',
    because:
      "rung 22 adds four columns across `chunk`, `entity`, `fact` and `page` — the four most content-bearing tables there are — and every one of them is a nullable integer, so the temptation rung 19 named and rungs 20 and 21 met is answered here by shape rather than by discipline: there is no room in an integer for a title, an excerpt or a provider's sentence about a document, and no later edit can widen one without changing the type in front of a reviewer. It is emphatically the tenant's database, because the stamp belongs on the row it is about and goes when that row goes. Each column records the consideration version at which one model phase last sent that row to the provider and got a readable answer back — written whether or not the row yielded a fact, a card, a conflict or a score, which is the whole design and the part a reviewer should check hardest. Keying it on output instead was refused on an ordinary case: a calendar invite states no factual claim, so under a yields-a-fact rule it is never done, and it sits at the top of the salience-ordered batch being re-sent every cycle forever with every chunk behind it waiting. The rung exists because four of the six model phases could say nothing about their own progress — their selectors took the top N by salience or by id with no clause about work already done — so the only thing between a second cycle and a second invoice was `consolidation_checkpoint`, a row keyed to one RUN, and a per-run row cannot be kept without keeping the run open, which is what left `extract` called once on a brain of 5,608 pages while every phase behind one unreadable page was never reached. `integer` rather than smallint for rung 21's reason, and nullable with no backfill for rung 17's: NULL means nobody has looked, which is the truth about every row that exists when this rung lands, and inventing a stamp would be claiming work no phase did. The indexes are plain btree rather than partial on NULL, because a partial index stops serving the predicate the moment a version is bumped, which is the one occasion the scan is over a whole corpus",
  },
  {
