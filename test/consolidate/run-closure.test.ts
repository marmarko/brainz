...
describe('a cycle stopped by a provider leaves no run open behind it', () => {
  test(
    ...
    async () => {
      ...
      // name for the rest of the brain's life. Nothing is open now.
      expect(openAfter).toEqual(Array.from({ length: CYCLES }, () => 0));

      // **And `[1, 1, 1]` is the whole argument for closing the run at all.**
      //
      // This assertion read `[1, 2, 3]` when closure was written on its own, and
      // that number is what got it reverted: an invoice for the same 5,608-page
      // corpus, every cycle, because the only record of extraction's progress
      // was a checkpoint row belonging to a run that no longer existed. It reads
      // 1 here without the run being held open for it, because the chunks are
      // stamped with the version that considered them — so a second cycle
      // selects nothing and never calls the provider. Closure costs nothing.
      //
      // It is still ONE rather than zero: the first cycle really did the work.
      expect(extractCalls).toEqual(Array.from({ length: CYCLES }, () => 1));
