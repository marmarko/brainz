 ...
 * against one global width, so a correct 1024 vector from the 1024 seat was
 * refused as malformed. `pendingChunkEmbeddings` asked `embedding IS NULL`, so
 * under a second seat the backlog was empty on the first day and every chunk was
 ...
 * answered, which is how a 1024 vector reaches a 1536 column: not as a wrong
 * answer but as a constraint violation halfway through a backfill, or — the same
 * width, some future seat — as no error at all.
 *
...
describe('a vector is measured against its own seat', () => {
  test('the shipped seat still refuses a vector of the other width', () => {
    expect(() => vectorLiteral(lexicalVector('x', QWEN.dimensions), ACTIVE_EMBEDDING_SEAT)).toThrow(
      EmbeddingWidthError,
    );
  });

  test('the 1024 seat accepts a 1024 vector and refuses a 1536 one', () => {
    const narrow = lexicalVector('x', QWEN.dimensions);
    expect(vectorLiteral(narrow, QWEN)).toStartWith('[');
    expect(() =>
      vectorLiteral(lexicalVector('x', ACTIVE_EMBEDDING_SEAT.dimensions), QWEN),
    ).toThrow(EmbeddingWidthError);
  });

  test('a non-finite component is refused under either seat', () => {
    const poisoned = lexicalVector('x', QWEN.dimensions);
    poisoned[0] = Number.NaN;
    expect(() => vectorLiteral(poisoned, QWEN)).toThrow(EmbeddingWidthError);
  });
});
...
describe('the backlog is per seat', () => {
  test('a chunk embedded by one seat is still pending for the other', async () => {
    ...
    const other = await pendingChunkEmbeddings(fixture.sql, 50, QWEN.column);
    ...
    expect(await backlogSize(fixture.sql, QWEN.column)).toBeGreaterThan(
...
describe('the backfill will not write a vector into another seat’s column', () => {
  test('draining the 1024 backlog against a 1536 gateway writes nothing', async () => {
    await seedChunk('a passage nothing has embedded yet');
    const { gateway } = createGateway();

    const before = await backlogSize(fixture.sql, QWEN.column);
    ...
    expect(await backlogSize(fixture.sql, QWEN.column)).toBe(before);
  });
  ...
  test('draining the shipped backlog against the same gateway does write', async () => {
    ...
    const rows = (await fixture.sql.unsafe(
      `SELECT ${ACTIVE_EMBEDDING_SEAT.column} IS NOT NULL AS shipped,
              ${QWEN.column} IS NOT NULL AS other
