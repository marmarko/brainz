...
async function storeWithClient() {
...
async function authorizeOnce(overrides: Partial<AuthorizeRequest> = {}) {
  const { store, clientId } = await storeWithClient();
...
describe('the authorization endpoint', () => {
  test('issues a code bound to the state and the redirect', async () => {
    const { outcome } = await authorizeOnce();
  ...
  test('refuses a redirect_uri that is not the registered string, exactly', async () => {
    for (const attack of [
      ...
      const { outcome } = await authorizeOnce({ redirectUri: attack });
  ...
  test('refuses a plain PKCE challenge — S256 is mandatory', async () => {
    const { outcome } = await authorizeOnce({ codeChallengeMethod: 'plain' });
    expect(outcome.ok).toBe(false);
  });

  test('refuses a missing PKCE challenge outright', async () => {
    const { outcome } = await authorizeOnce({ codeChallenge: '' });
    expect(outcome.ok).toBe(false);
  });

  test('refuses an unregistered client', async () => {
    const { outcome } = await authorizeOnce({ clientId: 'client-nobody-registered' });
    expect(outcome.ok).toBe(false);
    if (outcome.ok) return;
...
describe('redeeming the code', () => {
  async function issued() {
    const { store, clientId, outcome } = await authorizeOnce();
    if (!outcome.ok) throw new Error('fixture authorize failed');
    return { store, clientId, code: outcome.code };
  }

  test('the matching verifier redeems exactly once', async () => {
    const { store, clientId, code } = await issued();
    const first = await redeemAuthorizationCode(store, {
      code,
    ...
    const replay = await redeemAuthorizationCode(store, {
      code,
      codeVerifier: VERIFIER,
      redirectUri: REDIRECT,
      clientId,
  ...
  test('a code redeemed without the matching verifier is rejected', async () => {
    const { store, clientId, code } = await issued();
    for (const verifier of ['', 'not-the-verifier', CHALLENGE]) {
      const outcome = await redeemAuthorizationCode(store, {
        code,
        codeVerifier: verifier,
        redirectUri: REDIRECT,
        clientId,
  ...
  test('a code redeemed against an unregistered redirect_uri is rejected', async () => {
    const { store, clientId, code } = await issued();
    const outcome = await redeemAuthorizationCode(store, {
      code,
      codeVerifier: VERIFIER,
  ...
  test('a code redeemed by a different client is rejected', async () => {
    const { store, code } = await issued();
    const outcome = await redeemAuthorizationCode(store, {
      code,
      codeVerifier: VERIFIER,
      redirectUri: REDIRECT,
  ...
  test('a code past its short TTL is rejected, and the TTL really is short', async () => {
    const { store, clientId, code } = await issued();
  ...
  test('an unknown code is rejected without disclosing why', async () => {
    const { store, clientId } = await issued();
    const outcome = await redeemAuthorizationCode(store, {
...
describe('dynamic client registration', () => {
  test('a client whose redirect is outside the allowlist is refused', async () => {
    const store = createInMemoryAuthorizationStore();
  ...
  test('a client registering one allowed and one disallowed redirect is refused entirely', async () => {
    const store = createInMemoryAuthorizationStore();
    const outcome = await registerClient(
      store,
  ...
  test('registration is rate limited', async () => {
    ...
    expect((await registerClient(store, { clientName: 'a', redirectUris: [REDIRECT] }, options)).ok).toBe(true);
    expect((await registerClient(store, { clientName: 'b', redirectUris: [REDIRECT] }, options)).ok).toBe(true);
    const third = await registerClient(store, { clientName: 'c', redirectUris: [REDIRECT] }, options);
    expect(third.ok).toBe(false);
    if (third.ok) return;
    expect(third.error).toBe('rate_limited');

    // …and the window really is a window.
    const later = await registerClient(
      store,
  ...
  test('a registration with no redirect at all is refused', async () => {
    const store = createInMemoryAuthorizationStore();
    const outcome = await registerClient(
      store,
